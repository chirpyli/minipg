...
- [cryptohash.c](file://src/common/cryptohash.c)
- [cryptohash.h](file://src/include/common/cryptohash.h)
- [sha2.c](file://src/common/sha2.c)
- [system_functions.sql](file://src/backend/catalog/system_functions.sql)
</cite>

## 更新摘要
**所做更改**
- 移除了HMAC和SHA1加密函数的相关文档内容
- 更新了支持的加密函数列表，重点介绍MD5哈希（通过cryptohash路径）和SHA2系列函数
- 添加了新的加密函数实现章节，详细说明当前可用的哈希算法
- 更新了故障排查指南以反映移除的加密功能
...
- 加密函数实现：src/common/cryptohash.c, src/common/sha2.c
...
- [cryptohash.c:1-198](file://src/common/cryptohash.c#L1-L198)
...
- **新增**：加密函数库（cryptohash/sha2）：提供SHA224、SHA256、SHA384、SHA512等安全哈希算法实现。
...
- [spi.c:93-200](file://src/backend/executor/spi.c#L93-L200)
- [cryptohash.c:61-198](file://src/common/cryptohash.c#L61-L198)
...
### 7) 加密函数实现（新增）
**已更新** 移除了HMAC和SHA1函数支持，当前支持的加密函数包括：

- SHA2系列哈希函数
  - SHA224、SHA256、SHA384、SHA512
  - 通过 cryptohash 抽象层提供统一接口
  - 支持流式数据处理（update操作）
- MD5哈希函数
  - 通过cryptohash路径实现
  - 提供标准的MD5摘要计算
...
```mermaid
flowchart TD
Input["输入数据"] --> Create["pg_cryptohash_create()"]
Create --> Init["pg_cryptohash_init()"]
Init --> Update["pg_cryptohash_update()"]
Update --> MoreData{"更多数据?"}
MoreData -- 是 --> Update
MoreData -- 否 --> Final["pg_cryptohash_final()"]
Final --> Free["pg_cryptohash_free()"]
```

图表来源
- [cryptohash.c:61-198](file://src/common/cryptohash.c#L61-L198)
- [cryptohash.h:18-34](file://src/include/common/cryptohash.h#L18-L34)

章节来源
- [cryptohash.c:1-198](file://src/common/cryptohash.c#L1-L198)
- [cryptohash.h:1-37](file://src/include/common/cryptohash.h#L1-L37)
- [sha2.c:1-800](file://src/common/sha2.c#L1-L800)
...
- **新增**：加密函数依赖
  - cryptohash.c 提供统一的哈希函数抽象层
  - sha2.c 实现具体的SHA2算法
  - system_functions.sql 定义系统级哈希函数接口
...
CRYPTO["cryptohash.c"] --> EXEC
SHA2["sha2.c"] --> CRYPTO
SYSFUNC["system_functions.sql"] --> EXEC
```
...
- [spi.sgml:5002-5227](file://doc/src/sgml/spi.sgml#L5002-L5227)
- [cryptohash.c:151-198](file://src/common/cryptohash.c#L151-L198)

## 结论
PostgreSQL 的用户定义函数体系以 fmgr 为核心，配合 funcapi、SPI、执行器与目录系统，提供了从简单标量函数到复杂聚合/窗口/表值函数的完整能力。**经过更新后**，系统移除了HMAC和SHA1加密函数，专注于提供更安全的SHA2系列哈希算法和MD5哈希功能。通过正确理解函数注册、参数与返回约定、SPI 事务与结果集处理，以及聚合/窗口执行模型，可以高效地实现高性能、可维护的自定义函数。
...
- 快速参考
  ...
  - **新增**：加密函数：使用 cryptohash 抽象层进行SHA2系列哈希计算，MD5通过cryptohash路径实现。