...
- [src/backend/utils/adt/domains.c](file://src/backend/utils/adt/domains.c)
</cite>

## 更新摘要
**所做更改**
- 移除了关于域类型输入/输出函数（domain_in、domain_recv）的过时内容
- 更新了域类型相关章节，反映这些函数已被移除
- 移除了 ALTER TYPE SET SCHEMA 功能的引用
- 保留了核心域检查功能的描述
- 更新了相关代码示例和架构图
...
**注意**：域类型系统已进行重大更新，移除了 domain_in 和 domain_recv 函数，但保留了核心域检查功能。
...
- 域类型支持：domains.c 提供域类型的约束检查和验证功能
...
- [src/backend/utils/adt/domains.c:1-312](file://src/backend/utils/adt/domains.c#L1-L312)
...
- 域类型约束检查：通过 domains.c 提供的核心功能验证域类型约束
...
- [src/include/catalog/pg_type.h:47-189](file://src/include/catalog/pg_type.h#L47-L189)
- [src/backend/utils/adt/datum.c:16-41](file://src/backend/utils/adt/datum.c#L16-L41)
- [src/backend/utils/adt/domains.c:208-241](file://src/backend/utils/adt/domains.c#L208-L241)
...
participant Domain as "域类型检查<br/>domain_check"
...
Parser->>IO : "调用输入函数"
IO-->>Domain : "域类型时进行约束检查"
Domain-->>Datum : "返回验证后的 Datum"
...
- [src/backend/utils/adt/domains.c:214-241](file://src/backend/utils/adt/domains.c#L214-L241)
...
- **重要更新**：域类型不再使用 domain_in 和 domain_recv 函数，而是通过 domain_check 进行约束验证
...
### 域类型约束检查
**更新**：域类型系统已简化，移除了独立的 domain_in 和 domain_recv 函数，但保留了核心的域检查功能。

- 域类型通过 domain_check 函数进行约束验证
- 支持 NOT NULL 和 CHECK 约束检查
- 使用缓存机制提高性能，避免重复查询系统目录
- 错误信息包含详细的域名和约束名

```mermaid
flowchart TD
Input["输入值"] --> Validate{"域类型检查"}
Validate --> |NOT NULL| NotNull{"是否为NULL?"}
NotNull --> |是| Error1["抛出 NOT NULL 违反错误"]
NotNull --> |否| CheckConstraints["检查其他约束"]
Validate --> |CHECK| CheckExpr["执行 CHECK 表达式"]
CheckExpr --> |失败| Error2["抛出 CHECK 违反错误"]
CheckExpr --> |成功| Success["验证通过"]
Error1 --> End(["结束"])
Error2 --> End
Success --> End
```
...
**图表来源**
- [src/backend/utils/adt/domains.c:130-205](file://src/backend/utils/adt/domains.c#L130-L205)
- [src/backend/utils/adt/domains.c:214-241](file://src/backend/utils/adt/domains.c#L214-L241)
...
- **更新**：域类型转换现在通过 domain_check 进行，而不是通过独立的输入函数
...
Cast --> Domain{"是否域类型?"}
Domain --> |是| DomainCheck["执行 domain_check"]
DomainCheck --> Result["得到目标类型 Datum"]
Domain --> |否| Result
Direct --> Result
```
...
- 复合类型（如数组）：
  ...
  - 参考：arrayfuncs.c
- 域类型：
  - 利用 domain_check 进行约束验证
  - 参考：domains.c 中的约束检查逻辑
...
- **域类型优化**：利用缓存机制避免重复查询系统目录，提高约束检查性能
...
- **域类型问题**：检查 domain_check 是否正确调用，确认约束定义有效
...
- **域类型开发**：利用简化的 domain_check 接口进行约束验证，无需实现独立的输入/输出函数
...
- 关键 API 路径参考：
  ...
  - 域类型支持：[src/backend/utils/adt/domains.c](file://src/backend/utils/adt/domains.c)
