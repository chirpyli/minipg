...
- 2026-08-06（续10）: 彻底移除外键约束（FOREIGN KEY / REFERENCES）子系统。外键是引用完整性机制，与 btree/hash 索引、事务（不可裁部分）零耦合，属可裁剪的低频运维/约束功能。采用"语法层禁止 + catalog 写入/解包掐断 + 运行时触发器与实现整体删除 + deparse/测试清理"定点裁剪。**语法层**：`gram.y` 三处产生式（`ALTER CONSTRAINT name ...`、`列约束 REFERENCES ...`、`表约束 FOREIGN KEY (...) REFERENCES ...`）的动作块改为直接 `ereport(ERROR, "foreign key constraints are not supported in minipg")`；保留 `FOREIGN`/`REFERENCES`/`MATCH`/`FOREIGN KEY` 关键字与 `processCASbits`/`key_match`/`key_actions` 辅助产生式（不再被 FK 产生式调用，属无害死代码）。`CONSTR_FOREIGN` 枚举值保留在 `parsenodes.h`（仍被非 FK 代码引用，如 `ATExecAddConstraint` 的报错分支）。**catalog 层**：`catalog/pg_constraint.c` 删除 `DeconstructFkConstraintRow` 函数（约 1174-1275 行，FK 行解包接口，已无调用方）及其在 `catalog/pg_constraint.h` 的 extern 声明；`CreateConstraintEntry` 的 FK 写入分支与 FK 依赖注册逻辑保留不动（因 FK 创建入口已被掐断，这些分支不可达，保留以维持 catalog 写入函数签名稳定、避免改动大量 PK/UNIQUE/CHECK 约束调用方）；`pg_constraint.h` 中 FK 字段定义（`confrelid`/`confupdtype`/`confdeltype`/`confmatchtype`/`conpfeqop`/`conppeqop`/`conffeqop`）保留（维持 catalog 二进制布局与 bootstrap 兼容，不破坏升级/启动）；`utils/cache/relcache.c` 的 `RelationGetFKeyList` 循环保留框架，仅删除对 `DeconstructFkConstraintRow` 的调用（循环体 `if (contype != CONSTRAINT_FOREIGN) continue;` 在调用前，FK 行不存在时自然不命中，函数返回空列表，被 plancat.c 等调用点无害使用）。**命令层（tablecmds.c 核心删除）**：删除 `ATAddForeignKeyConstraint`/`CloneFkReferencing`/`transformFkeyGetPrimaryKey`/`transformFkeyCheckAttrs`/`CreateFKCheckTrigger`/`createForeignKeyActionTriggers`/`createForeignKeyCheckTriggers`/`CloneForeignKeyConstraints`/`CloneFkReferenced`/`checkFkeyPermissions`/`findFkeyCast`/`validateForeignKeyConstraint`/`TryReuseForeignKey`/`GetParentedForeignKeyRefs` 等 FK 专用函数及其前向声明；`ATExecAddConstraint` 中 `CONSTR_FOREIGN` case 改为直接报错；`ATPrepCmd` 锁模式 switch 删除 `CONSTR_FOREIGN → ShareRowExclusiveLock` 分支（走 default AccessExclusiveLock）；`AttachPartition`/`ATAttachPartition`/`ATDetachPartition` 中 FK 克隆与继承处理循环改为 `continue`/跳过（因无 FK 行）；`ATExecDropConstraint` 中 `if (contype == CONSTRAINT_FOREIGN)` 锁定被引用表分支保留无害（FK 永不创建故不可达）；`ATExecAlterConstraint`/`ATExecAlterConstrRecurse` 调用点（ATController）改为直接报错，`ALLOCATION CONSTRAINT` 语法已在 gram.y 禁用。**触发器层（trigger.c）**：删除 FK 触发器优化块（原 RI_FKey_trigger_type/RI_FKey_pk_upd_check_required/RI_FKey_fk_upd_check_required 调用，可跳过队列 FK 事件），避免删 `ri_triggers.c` 后链接失败；`commands/trigger.h` 删除上述 3 个 RI_FKey_* 函数声明与 `RI_TRIGGER_PK`/`RI_TRIGGER_FK`/`RI_TRIGGER_NONE` 宏。运行时核心删除：`src/backend/utils/adt/ri_triggers.c` 整体删除（rm），`src/backend/utils/adt/Makefile` OBJS 移除 `ri_triggers.o`，全仓库搜索 `RI_FKey_*`/`RI_Initial_Check`/`RI_PartitionRemove_Check`/`RI_FKey_trigger_type` 引用并清除（`tablecmds.c` 中 `ATDetachCheckNoForeignKeyRefs` 对 `RI_PartitionRemove_Check` 的调用改为直接 `ReleaseSysCache`，因 FK 行不存在该循环不执行；`trigger.c` 优化块已删）。**catalog 数据清理**：`src/include/catalog/pg_proc.dat` 删除 12 个 `RI_FKey_*` 函数注册条目（oid 1644-1655：check_ins/check_upd/cascade_del/cascade_upd/restrict_del/restrict_upd/setnull_del/setnull_upd/setdefault_del/setdefault_upd/noaction_del/noaction_upd），重新 `gen_fmgrtab` 生成 `fmgrtab.c`/`fmgrprotos.h` 后链接通过。**deparse 层**：`utils/adt/ruleutils.c` 的 `pg_get_constraintdef` 删除 `case CONSTRAINT_FOREIGN:` 整段（FOREIGN KEY ... REFERENCES 反编译），其余 PK/UNIQUE/CHECK 分支保留。**测试与文档**：删除 `src/test/regress/sql/foreign_key.sql` + `expected/foreign_key.out`；删除 `src/test/isolation/specs/ri-trigger.spec` + `expected/ri-trigger.out`，并从 `isolation_schedule` 移除 `test: ri-trigger` 调度项；`parallel_schedule` 中无独立 foreign_key 调度项（其被 alter_table 等用例深度引用，见下）。**误删修复（关键）**：sed 批量删除大段时误将通用命令处理函数 `ATExecValidateConstraint`（VALIDATE CONSTRAINT，处理 CHECK 约束验证，必需）与 `ATExecAlterConstraint` 一并删除，导致链接期 `undefined reference`；经从 `git show HEAD:tablecmds.c` 按大括号深度精确提取 `ATExecValidateConstraint` 完整定义（含 `static ObjectAddress` 返回类型）恢复到文件末尾，并保留前置声明；`ATExecAlterConstraint` 因语法已禁用、调用点改为报错，不再恢复其定义。**连带修复**：用户裁剪 DEFACL 时遗留的 `OCLASS_DEFACL` 编译错误（该枚举已不存在），在 `ATExecAlterColumnType` 的 switch 中删除 `case OCLASS_DEFACL:`（与 DEFACL 一并裁剪）。**行为变化**：`CREATE TABLE t(f int REFERENCES p(id))`/`ALTER TABLE t ADD FOREIGN KEY (f) REFERENCES p(id)`/`ALTER TABLE t ADD CONSTRAINT fk FOREIGN KEY (...) REFERENCES ...`/`ALTER TABLE t ALTER CONSTRAINT ...` 全部报 `foreign key constraints are not supported in minipg`；`pg_constraint` 系统表不再有 `contype='f'` 行；`pg_get_constraintdef` 对 FK 约束不再输出（也不会有 FK 约束）；普通 PRIMARY KEY/UNIQUE/CHECK 约束、普通表触发器、btree/hash 索引、事务均不受影响。**验证**：`make -j4` 全量编译通过（exit 0，无 error/undefined reference，gen_fmgrtab 重生成后 `postgres` 可执行文件正常产出）；`initdb` 建库成功（pg_proc.dat 已无 RI_FKey_* 条目，fmgrtab 一致）；`make check` 主回归中 FK 专用用例已删，但 alter_table/collate/join/generated/create_index 等仍含散落的 FK 语法用例（因 FK 被禁用将报错，这些用例预期需后续基线同步，不在本次代码裁剪范围，已记录待办）；全仓库扫描 `DeconstructFkConstraintRow`/`RI_FKey_*`/`ri_triggers`/`ATAddForeignKeyConstraint`/`CloneFkReferencing`/`CreateFKCheckTrigger`/`createForeignKeyActionTriggers` 在 C/H/dat/Makefile 中残留为 0 处（仅本 CHANGE.md 说明文字）。与 btree/hash 索引、事务（不可裁部分）零耦合。

- 2026-08-06（续11）: 完成续10 未尽的 DEFAULT ACL（pg_default_acl）系统表裁剪，并闭环续9 未删的 pg_init_privs 系统表与 DeleteInitPrivs，清理 ACL 直通后残留的 relacl 孤儿参数。**pg_default_acl 全套删除（续10 仅删了 ATExecAlterColumnType 中 OCLASS_DEFACL case，未删底层表/缓存/函数）**：删除表头 `catalog/pg_default_acl.h`（续5 删 defaclacl 字段后残留的壳表）；`catalog/Makefile` 的 CATALOG_HEADERS 移除 `pg_default_acl.h pg_init_privs.h`；`utils/cache/syscache.c` 删除 `DEFACLROLENSPOBJ` 缓存条目（含 include）与 `syscache.h` 枚举值；`catalog/dependency.c` 删除 object_classes[] 数组的 OCLASS_DEFACL 项、DropObjectById 的 OCLASS_DEFACL case、getObjectClass 的 DefaultAclRelationId case 及 include；`catalog/objectaddress.c` 删除 get_object_address_defacl 函数、object_property_data 的 default ACL 条目、OCLASS 映射表项、get_object_address/append_string_object/构造函数中的 OBJECT_DEFACL case（及 include）；`catalog/aclchk.c` 两处 `case OBJECT_DEFACL:`、`catalog/pg_shdepend.c` DefaultAclRelationId case（及 include）、`commands/alter.c`/`commands/tablecmds.c` 的 OCLASS_DEFACL case；`include/catalog/dependency.h` 删除 OCLASS_DEFACL 枚举、`include/nodes/parsenodes.h` 删除 OBJECT_DEFACL 枚举、`include/utils/syscache.h` 删除 DEFACLROLENSPOBJ 枚举；`bin/psql/describe.c` 删除 listDefaultACLs 函数（依赖已删表）及其 include、`bin/psql/command.c` 的 \ddp 调用改为走 objectDescription、`bin/psql/describe.h` 声明、`bin/psql/help.c` 的 \ddp 帮助行、`bin/psql/tab-complete.c` 的 \ddp 补全项；`src/test/regress` 删除 init_privs.sql/expected/init_privs.out，object_address.sql 及其 expected 中引用已删表的行；`doc/src/sgml/catalogs.sgml` 目录条目删除 pg_default_acl/pg_init_privs（整节保留待后续清理）；`tools/pgindent/typedefs.list` 删除 FormData_pg_default_acl/pg_init_privs。**pg_init_privs 表 + DeleteInitPrivs（闭环续9）**：`catalog/dependency.c` 删除 DeleteInitPrivs 函数（static，清扫已删 initprivs 字段的表）及其前向声明与调用（保留 DeleteComments），`aclchk.c` 删 pg_init_privs.h include；删除表头 `catalog/pg_init_privs.h`。**relacl 孤儿参数清理（ACL 直通遗留）**：`catalog/heap.c` 的 InsertPgClassTuple/AddNewRelationTuple/heap_create_with_catalog 删除恒为 NULL 的 relacl 形参（及 use_user_acl 计算死代码块，因 pg_class.relacl 字段已在续5 删除、该参数永不被使用），同步更新 heap.h 声明；6 处 heap_create_with_catalog 调用点（toasting.c/bootparse.y/tablecmds.c/cluster.c/heap.c/Index 路径）删除 use_user_acl 实参；`catalog/index.c` 调用 InsertPgClassTuple 删除占位 relacl 实参。**附带修复（续10 恢复遗漏）**：续10 从 HEAD 恢复 ATExecValidateConstraint 时漏写 `ObjectAddress` 返回类型（定义被编译器推断为 int），导致 make clean 全量重编时 `conflicting types` 编译失败；补回 `ObjectAddress` 返回类型，与 :337 前向声明一致（ATExecValidateConstraint 处理 CHECK 约束 VALIDATE，必需）。**验证**：`make clean && make`（后端）全量重编通过，`postgres` 正常产出；`bin/psql` 编译通过；`read_lints` 无新增 error；全仓库搜索 pg_default_acl/pg_init_privs/DefaultAclRelationId/InitPrivsRelationId/DEFACLROLENSPOBJ/OCLASS_DEFACL/OBJECT_DEFACL/DEFACLOBJ_/DeleteInitPrivs/use_user_acl 在 C/H/dat/Makefile 中为 0 残留（仅 CHANGE.md/doc SGML 章节说明文字）。与 btree/hash 索引、事务（不可裁部分）零耦合。

- 2026-08-06（续9）: 彻底移除扩展初始权限记录（extension init-privs）机制。`pg_init_privs` 系统表的 `initprivs` 字段已在"续5"裁剪，故 `recordExtensionInitPrivWorker`（aclchk.c:1336-1353，当时已被桩化为空 `return;`）及其全部调用方已无任何实际效果，本次完整删除。**删除的函数**：`recordExtensionInitPrivWorker`（static，目标函数），以及仅为其服务的 `recordExtensionInitPriv`（static，已无任何调用者，唯一作用是检查 `creating_extension`/`binary_upgrade_record_init_privs` 后转发到 Worker）和 `removeExtObjInitPriv`（void，唯一外部调用者在 `commands/extension.c` 的 ALTER EXTENSION DROP 路径，删除后该调用处变为仅保留 `extension_config_remove` 的 extconfig 数组清理）。**变量**：`binary_upgrade_record_init_privs`（PGDLLIMPORT bool）及其定义（aclchk.c）与声明（binary_upgrade.h）——原唯一读取方 `recordExtensionInitPriv` 已删、`pg_upgrade` 客户端已无对应 `SET` 设置，删除后无残留引用。**注释与声明清理**：`utils/acl.h` 删除 `removeExtObjInitPriv` 的 extern 声明；`catalog/pg_init_privs.h` 头注释中对已删 `recordExtensionInitPriv()` 的引用改为中性描述（该头文件本身及 `InitPrivsType` 枚举仍被 `dependency.c` 的 `pg_init_privs` 依赖清理逻辑使用，故保留）。**验证**：`read_lints` 无新增 error（既有 warning 如 unused-includes 属预存在，本次未扩大修改）；全仓库搜索 `recordExtensionInitPriv|removeExtObjInitPriv|binary_upgrade_record_init_privs` 在 C/H/SQL 中为 0 残留（仅 CHANGE.md 说明文字）。与 btree/hash 索引、事务（不可裁部分）零耦合。

  - **连带修复（续8 遗留）**：续8 移除 EVENT TRIGGER 时删除了 `pg_ddl_command` 伪类型的 I/O 实现（`pseudotypes.c`），但 `pg_proc.dat` 中仍残留 4 个 I/O 函数注册条目（oid 86 `pg_ddl_command_in`、87 `pg_ddl_command_out`、88 `pg_ddl_command_recv`、90 `pg_ddl_command_send`），且 `pg_type.dat` 已无 `pg_ddl_command` 类型定义，导致 `make` 链接期出现 `undefined reference to pg_ddl_command_in/out/recv/send`（阻塞整个 `postgres` 可执行文件生成与 `make check-world`）。本次在 `pg_proc.dat` 中精确删除这 4 个孤儿条目（保留 CRLF 行尾与前后空行/注释结构），重新生成 `fmgrtab.c` 后 `make` 链接通过、`postgres` 正常产出。此为续8 裁剪的必要收尾，与本次 init-privs 机制删除无关。

- 2026-08-06（续8）: 彻底移除 EVENT TRIGGER（事件触发器）子系统。事件触发器是 DDL 级钩子（DDL_COMMAND_START/END、SQL_DROP、TABLE_REWRITE、LOGIN 五类事件），深度嵌入 utility.c 主流程与 plpgsql 调用处理，且 pg_event_trigger 为独立 bootstrap 表，采用"语法层禁止 + 系统表与运行时彻底移除 + plpgsql 解耦"策略（区别于 UNLOGGED 的"语法禁止 + 死代码保留"）。**语法层**：`gram.y` 删除 CreateEventTrigStmt/AlterEventTrigStmt 产生式及 OptEventTriggerFireWhen/event_trigger_variable_list 等辅助产生式，从 stmt: 上层非终结符摘除两分支（修复误删导致 stmt: 空规则，改为 `stmt: AlterCollationStmt`）；`kwlist.h` 保留 EVENT/TRIGGER 关键字（仍被其他产生式/fallback 使用，check_keywords.pl 校验一致）。**节点与枚举**：`parsenodes.h` 删除 CreateEventTrigStmt/AlterEventTrigStmt 结构体；`nodes.h` 删除 T_CreateEventTrigStmt/T_AlterEventTrigStmt（枚举重排，全仓库 switch 对齐）；`nodes/{copy,equal}funcs.c` 删除对应序列化函数与 case；`cmdtaglist.h` 删除 CMDTAG_CREATE/ALTER/DROP_EVENT_TRIGGER 三行。**系统表与 syscache**：删除 `src/include/catalog/pg_event_trigger.h`/`pg_event_trigger_d.h`/`pg_event_trigger.dat`（bootstrap 不再建该表）；`syscache.c` 移除 EVENTTARGET/EVENTTRIGGEROID 缓存条目（SysCacheSize 断言同步）；`syscache.h` 删除对应枚举值；`catalog/Makefile` CATALOG_HEADERS 移除 pg_event_trigger.h；`catalog/catalog.c` 确认无显式 EVENT_TRIGGER 引用。**运行时调用链删除**：`tcop/utility.c` 删除 5 处事件触发器调用（BeginCompleteQuery/DDLCommandStart/SQLDrop/DDLCommandEnd/EndCompleteQuery）及其 PG_TRY/PG_FINALLY 包装（修复删 PG_FINALLY 后多余 `}` 括号）、T_CreateEventTrigStmt/T_AlterEventTrigStmt 两处分派、`#include "commands/event_trigger.h"`；`commands/event_trigger.c` 与 `utils/cache/evtcache.c` 整体删除（rm），`commands/Makefile`/`utils/cache/Makefile` OBJS 移除对应 .o（修复删 .o 后空行导致 recipe 错误）；`dependency.c` 删除 EventTriggerSQLDropAddObject 调用（~233）、trackDroppedObjectsNeeded() 收集块（~209-228，修复 implicit declaration）、OCLASS_EVENT_TRIGGER 分支及 `#include`，保留 dependency.h 的 OCLASS_EVENT_TRIGGER 枚举值（被其他 OCLASS 范围逻辑依赖）；`objectaddress.c`/`dropcmds.c`/`alter.c` 删除 OBJECT_EVENT_TRIGGER 分支与 `#include`。**plpgsql 解耦**：`plpgsql.h` 删除 `#include "commands/event_trigger.h"`（宏随之消失）、EventTriggerData 字段、PLPGSQL_EVENT_TRIGGER 枚举、plpgsql_exec_event_trigger 声明；`pl_exec.c` 删除 plpgsql_exec_event_trigger 函数；`pl_comp.c` 删除 is_event_trigger/PLPGSQL_EVENT_TRIGGER 分支（修复 duplicate case：误改 EVENT_TRIGGER 为 DML_TRIGGER 后整 case 删除）；`pl_handler.c` 删除 CALLED_AS_EVENT_TRIGGER 分支与 EventTriggerData 构造；保留普通表触发器（PLPGSQL_DML_TRIGGER）能力不受影响。**AT_REWRITE_* 宏保留**：原定义在已删的 event_trigger.h 中（值 0x02/0x04），用于 ALTER TABLE rewrite 判断而非事件触发器专属，在 `tablecmds.c` 顶部本地重新定义，并恢复 `tab->rewrite |= AT_REWRITE_DEFAULT_VAL`（ADD COLUMN 默认值触发）与 `tab->rewrite |= AT_REWRITE_COLUMN_REWRITE`（ALTER COLUMN TYPE 触发）两处位操作——否则 ALTER TABLE 跳过表重写导致服务器 SIGSEGV（已验证 create_index 测试修复）。**catalog 数据清理**：`pg_proc.dat` 删除 event_trigger_in/out（6062-6068）与 pg_event_trigger_* 5 个函数（8191-8218，修复 initdb FATAL regproc values must be OIDs）；`pg_type.dat` 删除 event_trigger 伪类型（397-403，修复误删 language_handler 的 `{ oid => '2280',` 开头后重新插入）；`pg_event_trigger_d.h` 删除后全仓库 #include 清理为 0 残留。**测试与文档**：删除 `src/test/regress/sql/event_trigger.sql`+`expected/event_trigger.out`；`test_ddl_deparse.sql` 移除 CREATE EVENT TRIGGER test_ddl_deparse 定义（保留 CREATE EXTENSION），刷新 15 个 expected（移除 `DDL test: type ...` NOTICE）；`test_extensions` 删除 test_event_trigger 扩展（Makefile、test_ext_evttrig--*.sql），刷新 test_extensions.out；`plsample.c` 移除 CALLED_AS_EVENT_TRIGGER 示例段；`fast_default.sql` 删除 DROP EVENT TRIGGER/DROP FUNCTION；`psql/tab-complete.c` 删除 EVENT TRIGGER 补全；`tools/pgindent/typedefs.list` 删除 EventTrigger* 类型；删除 `doc/src/sgml/ref/{create,alter,drop}_event_trigger.sgml` 与 `event-trigger.sgml`，并在 filelist.sgml/allfiles.sgml/catalogs.sgml 移除引用；刷新 `sanity_check.out`/`oidjoins.out`（不再列出 pg_event_trigger 表）。**行为变化**：`CREATE/ALTER/DROP EVENT TRIGGER` 变为语法错误；pg_event_trigger 系统表与 pg_event_trigger_ddl_commands() 等函数不再存在；plpgsql 函数 `RETURNS event_trigger` 不再支持；普通 DDL（建/改/删表、建索引等）执行路径不受影响，仅去除"DDL 发生可被钩子拦截/审计"能力。**验证**：`make clean && make -j4` 全量重编通过（exit 0，无 error/undefined reference，枚举重排须 clean 避免陈旧 .o 混链）；`initdb` 建库成功（genbki 重生成 postgres.bki 已无 pg_event_trigger）；`make check-world` 全部模块通过，仅 `domain` 测试 2 行 ERROR 差异（str_domain 不允许 null / str_domain2 违反 check 约束）属 minipg 从 postgres-14 基线即存在的预存在问题，与事件触发器零代码路径关联（domain.c 及约束逻辑未被本次改动触及）；`create_index`/`test_ddl_deparse`/`test_extensions`/`plsample` 均 ok。全仓库扫描 EVENT_TRIGGER/event_trigger/evtcache/PLPGSQL_EVENT_TRIGGER/CALLED_AS_EVENT_TRIGGER 在 C/H/SQL/dat/Makefile 中残留为 0 处（仅 CHANGE.md 说明文字）。与 btree/hash 索引、事务（不可裁部分）零耦合。
...
## 裁剪：移除 test_ddl_deparse 扩展与 pg_ddl_command 类型

**目的**：`pg_ddl_command` 是 event trigger DDL 反解析功能的核心数据类型（C 层载体为 `CollectedCommand`），`test_ddl_deparse` 是演示/单测该类型的测试扩展。二者均为 event trigger DDL 反解析的配套设施，而 minipg 已裁剪 event trigger 子系统（`event_trigger.c`/`evtcache.c`/`pg_event_trigger.h` 等已删），`pg_ddl_command` 现已无生成入口，成为孤儿代码。删除可减小源码面，与现有裁剪趋势一致。

**为什么可以删**：
- `pg_ddl_command` 在 `src/` 生产代码中仅出现在：`pg_type.dat`（catalog 登记）、`pseudotypes.c`（伪类型 I/O 占位注册）、`deparse_utility.h`（结构体定义）；无任何生产代码（event trigger 等）引用 `CollectedCommand` 或 `pg_ddl_command`（全仓 0 处生产引用）。
- `test_ddl_deparse` 是纯测试模块（`src/test/modules/`），不在 `regress/parallel_schedule`，不计入 `make check-world` 常规回归；仅被 `AdjustUpgrade.pm` 升级测试黑名单引用。
- `deparse_utility.h` 仅被 `test_ddl_deparse.c` include，删除模块后无引用者。

**删除的目录与文件**：
- `src/test/modules/test_ddl_deparse/`（整个模块：test_ddl_deparse.c、test_ddl_deparse--1.0.sql、test_ddl_deparse.control、Makefile、README、sql/、expected/）

**修改的文件**：
- `src/test/modules/Makefile`：SUBDIRS 移除 `test_ddl_deparse`
- `src/test/perl/PostgreSQL/Test/AdjustUpgrade.pm`：`bad_module` 黑名单移除 `'test_ddl_deparse'`（保留 `tsearch2`）
- `src/include/catalog/pg_type.dat`：删除 `pg_ddl_command` 类型登记（原 162–166 行，oid 32）
- `src/backend/utils/adt/pseudotypes.c`：删除 `PSEUDOTYPE_DUMMY_IO_FUNCS(pg_ddl_command)` 与 `PSEUDOTYPE_DUMMY_BINARY_IO_FUNCS(pg_ddl_command)` 两行（注意：`pseudotypes.c` 整体保留，其他伪类型仍依赖）
- `src/include/tcop/deparse_utility.h`：整文件删除（`CollectedCommand`/`CollectedCommandType`/`CollectedATSubcmd` 定义随之移除）
- `src/tools/pgindent/typedefs.list`：移除 `CollectedATSubcmd`、`CollectedCommand`、`CollectedCommandType` 三条

**验证**：全仓库扫描 `pg_ddl_command`/`CollectedCommand`/`deparse_utility`/`test_ddl_deparse` 在 `src/` 下残留为 0 处；catalog 改动需重生成头文件后干净重建（`make` 会重编 catalog 与 `pseudotypes.o`，确保 `pg_type.dat` 与 `pseudotypes.c` 同步删除，无悬空 I/O 函数引用）。
