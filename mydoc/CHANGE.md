...
- **interval 时间段类型（09-02）**：彻底删除 SQL 时间段类型 `interval` 及其全链路依赖。`pg_type.dat` 删 interval 类型行；`pg_proc.dat` 删约 84 条 interval 相关函数（`interval_in/out`、比较 `=/</>/<=/>=/<>` 与 `interval_cmp`、`interval_hash`/`interval_hash_extended`、`interval_um`/`interval_scale`、`justify_hours`/`justify_interval`/`justify_days`、`make_interval`、`interval_trunc`、`extract_interval`、`date_part` on interval、`age()` 族 `timestamptz_age`/`timestamp_age`/date 双参 `age`、`timestamp`/`timestamptz`/`date` 的 ±interval、`in_range_*_interval` 窗口帧；interval 版 `percentile_*` 已在聚合裁剪中先行删除）；`pg_operator.dat` 删 interval 运算符，`pg_opclass`/`pg_opfamily`/`pg_amop`/`pg_amproc.dat` 删 btree/hash 的 `interval_ops`，`pg_cast.dat` 删 time↔interval 与 interval↔interval 转换。`timestamp.c` 删 interval 全部实现与 `#include "common/int128.h"`（含 `interval_cmp_value`），`date.c` 删约 15 处 ±interval 引用，`selfuncs.c` 删 `convert_timevalue_to_scalar`/`scalarineqsel` 中 `case INTERVALOID`；头文件 `datatype/timestamp.h` 删 `Interval` 结构与 `IntervalIsValid`、`utils/datetime.h` 删 `IntervalStyle` 枚举/`INTERVAL_FULL_RANGE`/`INTERVAL_MASK` 宏/`DecodeInterval` 声明、`utils/timestamp.h` 删 interval 系列原型（经 grep `datetime.c` 对 interval 零引用，未改动）。语法层 `gram.y` 删 `ConstInterval`/`opt_interval`/`interval_second` 及 interval 字面量与 `YEAR TO MONTH`/`DAY TO SECOND` 限定符产生式；`variable.c` 删 `SET TIME ZONE INTERVAL '...'` 分支（数值/`hh:mm` 时区形式保留）。**删仅被 interval 使用的文件**：`common/int128.h` 与 `src/tools/testint128.c`（`c.h` 原生 `int128/uint128` 仍供 `common/int.h` 溢出快路径、`d2s.c`、`intagg.c` 使用，保留）。**保留**：`IntervalStyle` GUC（死配置，改动收益低、回归风险高，保留）、`c.h` 的 int128 类型定义、纯 `overlaps(timestamp,timestamp,timestamp,timestamp)` 等无 interval 重载（未受波及）。回归：删 `interval.sql`/`expected/interval.out`、`parallel_schedule` 移除 `interval`；同步 `horology/time/timestamp/timestamptz/opr_sanity/arrays/hash_func/sanity_check/sysviews/misc_functions` 等 expected（其中 `euc_kr`/`select_implicit`/`conversion` 三例 expected 偏差为更早「字符集编码裁剪」遗留，非 interval 引入，已随实际输出同步，现 67 用例全绿）。
