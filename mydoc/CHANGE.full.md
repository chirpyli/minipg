...
## 裁剪 to_char / to_number / to_timestamp 数据格式化功能（2026-09-01）

### 一、背景
Oracle 风格 `TO_CHAR()/TO_DATE()/TO_NUMBER()` 兼容层（`src/backend/utils/adt/formatting.c`，6418 行），提供日期时间/间隔/数值的模板化格式化与反向解析（DCH 关键字表、NUM 数字处理器、罗马数字、序数后缀、本地化月份/星期名缓存），属纯用户态表达层便利特性，与 btree/hash 索引、事务零耦合，学习内核价值低。本轮整文件删除，连同仅服务该功能的 `lc_time` 本地化缓存与 GUC、`func.sgml` 格式化函数整节一并移除；`lower()/upper()/initcap()` 依赖的大小写转换辅助函数迁移到唯一消费方 `oracle_compat.c`。

### 二、删除/更新内容
- 删除 `src/backend/utils/adt/formatting.c` 与 `src/include/utils/formatting.h`；`src/backend/utils/adt/Makefile` 移除 `formatting.o`。
- 迁移：`formatting.c` 的大小写转换函数 `str_tolower/str_toupper/str_initcap` 与 `asc_tolower/asc_toupper/asc_initcap` 迁入 `oracle_compat.c`，`asc_*` 收窄为 `static`；删除无调用方的 `*_z` 便捷包装。`oracle_compat.c` 新增 `catalog/pg_collation.h`、`utils/pg_locale.h`、`<limits.h>`、`<wchar.h>`/`<wctype.h>` 等 include，并补前向声明。
- `pg_proc.dat`：用 `sed -i` 删除 10 条 SQL 入口（`to_char`×7：`timestamptz_to_char`/`int4_to_char`/`int8_to_char`/`float4_to_char`/`float8_to_char`/`timestamp_to_char`/`interval_to_char`，以及 `to_timestamp(text,text)`、`to_date`；`to_number` 本就无 `pg_proc` 条目，随 C 代码删除），及 `# formatting` 注释行。`to_timestamp(float8)`（Unix epoch 转换）、`pg_encoding_to_char` 保留。
- `lc_time` 整链删除：`pg_locale.c` 删除 `locale_time` 变量、`localized_abbrev_days/full_days/abbrev_months/full_months` 四个缓存数组、`check_locale_time/assign_locale_time`、`cache_locale_time`、`cache_single_string`、`case LC_TIME` 分支、`MAX_L10N_DATA` 宏及 `<time.h>` include；`pg_locale.h` 删除对应声明；`guc.c` 删除 `lc_time` GUC 定义；`main.c` 删除 `init_locale("LC_TIME", ...)`；`postgresql.conf.sample` 删除 `#lc_time = 'C'`；`pg_regress.c` 删除 `unsetenv("LC_TIME")` 与 `SET lc_time TO 'C';`；`initdb.c` 不再向 `postgresql.conf` 写 `lc_time`（其 `--lc-time` 选项与 `locale_date_order()` 保留，继续驱动 `datestyle` 初始化）。
- 回归测试：删除/改写引用被裁函数的用例——`int8.sql`/`timestamp.sql`/`timestamptz.sql`/`horology.sql` 整段 `to_char`/`to_date`/`to_timestamp` 测试；`psql_crosstab.sql` 的 `to_char(d,'Mon')` 改为 `EXTRACT(month FROM d)` 月份名数组；`text.sql`/`polymorphism.sql` 的 `to_date('...','YYYYMMDD')` 改为 `::date`；对应 `expected/*.out` 用实际运行结果回写。
- 文档：`doc/src/sgml/func.sgml` 删除 `id="functions-formatting"` 整节（Data Type Formatting Functions），并修正 3 处 `<xref linkend="functions-formatting"/>`（functions-datetime/extract/datatype 章节）及若干 `to_char`/`to_timestamp(template)` 表述；`config.sgml` 删除 `guc-lc-time` varlistentry；`charset.sgml` 删除 `LC_TIME` 环境变量表项；`datatype.sgml`/`features.sgml` 删除失效 xref。

### 三、验证
- 全量重编：`env -i PATH=/usr/local/bin:/usr/bin:/bin:/usr/local/sbin:/usr/sbin HOME=/home/postgres ./configure --prefix=/home/postgres/minipg --enable-debug && make -j`（catalog `.dat` 变更经 genbki 自动重生成 `fmgroids.h`/`fmgrtab.c`）。
- `make check-world` 全部套件通过（regress 68 个、isolation 58 个等，零失败）；所有 `expected/*.out` 已同步，`to_timestamp(float8)` 与 `pg_encoding_to_char` 相关用例保持通过。
- 全代码库 grep `to_char|to_date|to_number|to_timestamp|formatting.c|cache_locale_time|localized_|lc_time|LC_TIME` 在 `src/` 下，除 `initdb.c` 的 `--lc-time`/DateStyle 探测、`timezone/strftime.c` 无关的 `lc_time_T` 结构体、`pg_encoding_to_char` 外均零命中。
