...
SELECT regoperator('++(int4,int4)');
SELECT regproc('know');
SELECT regprocedure('absinthe(float8)');
...
SELECT regprocedure('ng_catalog.abs(float8)');
...
SELECT to_regprocedure('absinthe(float8)');
...
SELECT to_regprocedure('ng_catalog.abs(float8)');
