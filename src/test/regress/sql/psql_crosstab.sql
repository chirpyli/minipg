...
SELECT v, (array['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'])[EXTRACT(month FROM d)::int] AS "month name", EXTRACT(month FROM d) AS num,
...
SELECT EXTRACT(year FROM d) AS year, (array['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'])[EXTRACT(month FROM d)::int] AS """month"" name",
  EXTRACT(month FROM d) AS month,
  format('sum=%s avg=%s', sum(i), avg(i)::int8(2,1))
  FROM ctv_data
  GROUP BY EXTRACT(year FROM d), 2, EXTRACT(month FROM d)
ORDER BY month
