...
INSERT INTO tidrangescan SELECT i,repeat('x', 100) FROM (SELECT generate_series(1,200) AS i) AS s;
...
WHERE substring(ctid::text, position(ctid::text, ',') + 1, length(ctid::text) - position(ctid::text, ',') - 1)::integer > 10 OR substring(ctid::text, 2, position(ctid::text, ',') - 2)::integer > 2;
VACUUM tidrangescan;
