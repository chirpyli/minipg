 ...
 * just a copy of the relevant es_relations[] entry.  In situations where we
 * build a ResultRelInfo for a relation that is not in the range table,
 * ri_RangeTableIndex is 0 and ri_RelationDesc is a separately-opened
 * relcache pointer that needs to be separately closed.
 *
 * (minipg has cropped the two cases that used to need this: tuple routing
 * into a partitioned table, and firing triggers on other tables.)
 */
