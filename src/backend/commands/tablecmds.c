...
#define AT_PASS_COL_ATTRS		6	/* set column attributes, eg compression */
...
static void ATPostAlterTypeCleanup(List **wqueue, AlteredTableInfo *tab,
								   LOCKMODE lockmode);
static void ATPostAlterTypeParse(Oid oldId, Oid oldRelId,
...
/*
 * RemoveRelations
 *		Implements DROP TABLE, DROP INDEX, DROP VIEW
 */
void
 ...
 * constraints will be removed from the
				...
				AlterTableUtilityContext *context)
{
	...
	collOid = GetColumnDefCollation(typeOid);
 ...
 */
static void
ATPrepAlterColumnType(AlteredTableInfo *tab, Relation rel,
					  AlterTableCmd *cmd)
{
	...
	if (tab->relkind == RELKIND_RELATION)
	{
		...
		if (!transform)
		{
			transform = (Node *) makeVar(1, attnum,
											...
											get_typcollation(attTup->atttypid),
											0);
		}
...
ATPostAlterTypeCleanup(List **wqueue, AlteredTableInfo *tab, LOCKMODE lockmode)
{
			...
			def_item, tab->changedConstraintDefs)
	{
		...
		ATPostAlterTypeParse(oldId, relid,
 ...
 */
static void
ATPostAlterTypeParse(Oid oldId, Oid oldRelId, char *cmd,
 ...
 * TABLE; we expose it here so that it can be
 * used by all.
 */
void
